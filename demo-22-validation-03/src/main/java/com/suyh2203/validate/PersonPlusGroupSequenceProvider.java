...
/**
 * 该接口定义了：动态Group序列的协定
 * 要想它生效，需要在{@link PersonPlus}上标注@{@link GroupSequenceProvider} 注解并且指定此类为处理类
 * 如果{@link Default}组对{@link PersonPlus}进行验证，则实际验证的实例将传递给此类以确定默认组序列（这句话特别重要  下面用例子解释）
 */
@Slf4j
public class PersonPlusGroupSequenceProvider implements DefaultGroupSequenceProvider<PersonPlus> {
     ...
     */
    @Override
    public List<Class<?>> getValidationGroups(PersonPlus bean) {
